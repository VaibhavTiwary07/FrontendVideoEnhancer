...
/// Singleton logger for export diagnostics
/// Writes all export-related logs to a file for easy debugging
final class ExportDiagnostics {
    static let shared = ExportDiagnostics()

    private let fileManager = FileManager.default
    private let queue = DispatchQueue(label: "com.export.diagnostics", attributes: .concurrent)
    ...
    private var logFileURL: URL {
        ...
        return documentsPath.appendingPathComponent("export_debug.log")
    }

    // Unified logging for iOS device console (visible in Xcode and Console app)
    private let osLog = OSLog(subsystem: "com.videoenhancer.export", category: "Diagnostics")
    ...
    private init() {
        ...
        DispatchQueue.main.async {
            ...
            let deviceInfo = """
            ═══════════════════════════════════════════════════════════════
            EXPORT DIAGNOSTICS LOG STARTED
